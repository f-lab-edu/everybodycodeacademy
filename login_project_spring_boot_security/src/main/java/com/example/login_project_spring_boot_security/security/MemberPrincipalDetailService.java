...
public class MemberPrincipalDetailService implements UserDetailsService {
  ...
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    ...
    // 없을경우 에러 발생
    if (member == null) throw new UsernameNotFoundException(username + "을 찾을 수 없습니다.");

    if (!"Y".equals(member.getIsUsed())) throw new UsernameNotFoundException("사용할 수 없는 계정입니다.");

    if (!"N".equals(member.getIsDel())) throw new UsernameNotFoundException("삭제된 계정입니다.");
